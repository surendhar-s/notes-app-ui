import { Button, InputBase, MenuItem, Paper, Popper } from '@material-ui/core';
...
class Home extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      getShareNoteType: "SHARED WITH YOU",
  ...
  getNoteData = async (userId) => {
    ...
    if (this.state.noteType === "Shared") {
      if (this.state.getShareNoteType === "SHARED WITH YOU") {
      ...
      else if (this.state.getShareNoteType === "SHARED BY YOU") {
  ...
  getNotificationsData = async (userId) => {
    const data = await axios.get("http://localhost:3200/api/users/getNotificationByUserId/" + userId,
      { headers: { "Authorization": `Bearer ${this.Auth.getToken()}` } })
    this.setState({ notificationsData: data.data.message.notificationsList.notificationsList })
  }
  ...
  componentDidMount() {
    ...
    if (!this.Auth.loggedIn()) {
      ...
      try {
        ...
        this.getNoteData(decode(this.Auth.getToken()).userId)
  ...
  handleGetShareNoteType = (e) => {
    this.setState({
      getShareNoteType: e.target.innerText
    }, () => {
      this.getNoteData(decode(this.Auth.getToken()).userId)
    })
  }

  handleNewNoteNotification = (data) => {
    console.log(data)
  }

  handleNotificationIconClick = () => {
    this.setState({ notificationIcon: !this.state.notificationIcon })
  }
  ...
  handleNotificationAction = (e) => {
    if(e.target.innerText === "Delete"){
      ...
      // const data = axios.post("http://localhost:3200/api/users/deleteNotification",{
      //   userId: decode(this.Auth.getToken()).userId,
      //   noteId: 
      // })
    }
    else if(e.target.innerText === "Mark as read"){
      console.log("Implement read function")
    }
  }
            ...
            >
              <Toolbar className="header-con">
                <div style={{ display: "flex" }}>
                  ...
                  <img src="assests/logo.png" width="40" height="40" alt="App Logo" />
                  ...
                  <div style={{ marginTop: "10px" }}>
                    <IconButton
                      ...
                      className={clsx(classes.expand, {
                        [classes.expandOpen]: this.state.notificationIcon,
                      })}
                      onClick={this.handleNotificationIconClick}
                      aria-expanded={this.state.notificationIcon}>
                      <Badge badgeContent={this.state.notificationsData === null ? 0 : this.state.notificationsData.length} color="secondary">
                        <NotificationsIcon />
                      </Badge>
                      <Popper style={{ width: "280px" }} open={this.state.notificationIcon} placement="top" role={undefined} disablePortal >
                        <Paper className="paper-option">
                          {/* <ClickAwayListener onClickAway={this.handleNewNoteNotification}> */}
                          {/* <MenuList autoFocusItem={this.state.notificationIcon} id="menu-list-grow">
                              {
                                this.state.notificationsData !== null ?
                                  this.state.notificationsData.map((data) =>
                                    <MenuItem key={Math.random()} onClick={this.handleNewNoteNotification}>{data.notificationTitle}</MenuItem>
                                  )
                                  : <MenuItem >You have no new notifications!</MenuItem>
                              }
                            </MenuList> */}
                          {this.state.notificationsData !== null ? <div> {this.state.notificationsData.length !== 0 ?
                            this.state.notificationsData.map((data) =>
                              <CardHeader key={Math.random()} onClick={this.handleNewNoteNotification} avatar={<Avatar aria-label="recipe" className={classes.avatar}>{data.notificationTitle.charAt(0).toUpperCase()}</Avatar>} title={data.notificationTitle} subheader={<div style={{ display: "flex", justifyContent: "space-between" }}><div>{this.getFormattedDateTime(data.dateShared)}</div><div><span onClick={this.handleNotificationAction}>Delete</span></div><div><span onClick={this.handleNotificationAction}>Mark as read</span></div></div>} />
                            )
                            : <CardHeader title="You have no new notifications!" />} </div> :
                            <CardHeader title="You have no new notifications!" />
                          }
                          {/* </ClickAwayListener> */}
                        </Paper>
                      </Popper>
              ...
              <LoadingOverlay
                ...
                <div>
                  ...
                  {this.state.noteData !== null ?
                    <div>
                      {this.state.noteType === "Notes" ? <span style={{ fontSize: "25px", fontWeight: "bold", marginLeft: "20px" }}>All Notes</span> : <span></span>}
                      ...
                      {this.state.noteType === "Shared" ? <div>
                        ...
                        <Button color="secondary" size="small" onClick={this.handleGetShareNoteType}>SHARED WITH YOU</Button><br />
                        {this.state.getShareNoteType === "SHARED BY YOU" ? <b style={{ marginLeft: "6px" }}>Notes that you've shared</b> : <b style={{ marginLeft: "6px" }}>Notes shared with you</b>}
                      </div> : <div></div>}
                      {this.state.noteData.length !== 0 ? <div>
                        <div className="note-con">
                          {
                            this.state.noteData.map((data, index) => {
                              return <Notes noteData={data} auth={this.Auth} isShared={this.state.noteType === "Shared" && this.state.getShareNoteType === "SHARED WITH YOU"} overRideShare={this.state.getShareNoteType === "SHARED BY YOU"} createrName={this.state.createrNames[index]} key={Math.random()} />
                            })
                          }
                        </div>
